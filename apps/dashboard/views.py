...
def stock_view(request):
    ...
    stocks = (
        Stock.objects.select_related('product__category', 'product__brand')
        .order_by('quantity')
    )
...
def cash_register_view(request):
    ...
    transactions = open_cash.transactions.order_by('-created_at') if open_cash else []
...
# ── Pedidos (views legadas, mantidas para compatibilidade de URLs) ──

@staff_member_required
def orders_view(request):
    """Lista de pedidos — compatível com {% url 'dashboard:orders' %}."""
...
def order_detail_staff_redirect(request, order_number):
    """Atualiza status via POST e redireciona — mantém URL legada funcionando."""
