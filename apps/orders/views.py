...
from django.db import transaction  # IMPORTANTE: Adicionado para a atomicidade
...
def checkout_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            # O bloco atomic garante que ou TUDO é salvo, ou NADA é salvo.
            with transaction.atomic():
                ...
                for item in cart.items.select_related('product__stock').all():
                    # Validação de segurança: Impede a venda se o estoque secou no meio do checkout
                    if item.product.stock.quantity < item.quantity:
                        raise ValueError(f"O produto '{item.product.name}' não possui estoque suficiente ({item.product.stock.quantity} disponíveis).")
                    ...
                    # Baixa o estoque de forma segura com F()
                    item.product.stock.quantity = F('quantity') - item.quantity
                    item.product.stock.save(update_fields=['quantity'])

                # Esvazia o carrinho apenas se a transação dos itens deu certo
                cart.items.all().delete()

            # Se chegou aqui, o commit foi feito com sucesso no banco de dados
            messages.success(
                request,
            ...
            # Captura o erro de falta de estoque e avisa o usuário (rollback automático acionado)
            messages.error(request, str(e))
            return redirect('orders:cart')
        except Exception as e:
            # Qualquer erro no banco faz o rollback e redireciona com segurança
